...
{ 
	...
	//	int nMin = 2000000, nMax = -2000000;
		...
		if( nPiece == m_nSearchPiece )
		{
			...
			int nValue = (int) Float.NEGATIVE_INFINITY; 
			...
			for( int i=0; i<nMoves; i++ )
			{
				...
				nValue = nResultList[i]= (int)Math.max(nValue, DoSearch( pos, nPiece ^ 1, nDepth + 1, pBoard, nAlpha, nBeta ));
				
				nAlpha = Math.max(nValue, nAlpha);			
				
				if (temp < nValue &&  nDepth == 0)
				{
		...
		{
			int nValue = (int)Float.POSITIVE_INFINITY;
			...
			for( int i=0; i<nMoves; i++ )
			{
				...
				// Call DoSearch() recursively.
				nValue = nResultList[i]= (int)Math.min(nValue, DoSearch( pos, nPiece ^ 1, nDepth + 1, pBoard, nAlpha, nBeta ));
				
				nBeta = Math.min(nValue, nBeta);			
	...
	public void GetMove( Position pos, int[][] BoardData, int nPiece )
	{
		...
		int nAlpha = (int)Float.NEGATIVE_INFINITY;
		int nBeta = (int)Float.POSITIVE_INFINITY;
